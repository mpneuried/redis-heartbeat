(function() {
  ...
  Heartbeat = (function(superClass) {
    extend(Heartbeat, superClass);
    ...
    function Heartbeat(options) {
      ...
      this._getRedisTime = bind(this._getRedisTime, this);
      this._getTime = bind(this._getTime, this);
      this._content = bind(this._content, this);
      this._send = bind(this._send, this);
      this.metrics = bind(this.metrics, this);
      this.heartbeat = bind(this.heartbeat, this);
    ...
    Heartbeat.prototype._start = function() {
      ...
      this.active = true;
      if (!((ref = this.config.name) != null ? ref.length : void 0)) {
        this._handleError(false, "ENONAME");
        return;
      }
      if (!lodash.isFunction(this.config.identifier) && !((ref1 = this.config.identifier) != null ? ref1.length : void 0)) {
    ...
    Heartbeat.prototype._content = function() {
      ...
      this._getTime((function(_this) {
        return function(err, ms) {
          var _data, _iden, _key, _sData, _statements, ald15m, ald1m, ald5m, ref;
          if (err) {
            cb(err);
            return;
          }
          ...
          if (type === "metric") {
            ...
            ref = os.loadavg(), ald1m = ref[0], ald5m = ref[1], ald15m = ref[2];
            _data = {
              t: ms,
            ...
            _statements.push(["LTRIM", _key, 0, _this.config.metricCount - 1]);
            if (_this.config.metricExpire > 0) {
              _statements.push(["EXPIRE", _key, _this.config.metricExpire]);
            }
            cb(null, _statements);
            return;
          }
    ...
    Heartbeat.prototype._getTime = function(cb) {
      if (this.config.localtime || !this.connected) {
        cb(null, Date.now());
        return;
      }
